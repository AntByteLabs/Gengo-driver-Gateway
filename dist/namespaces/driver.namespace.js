 ...
 */
export function setupDriverNamespace(io) {
    const driverNsp = io.of('/driver');
    // ── Auth middleware ───────────────────────────────────────────────────────
    driverNsp.use(createSocketAuthMiddleware('driver'));
    // ── Connection handler ────────────────────────────────────────────────────
    driverNsp.on('connection', async (socket) => {
        ...
        if (handshakeTripId && handshakeTripId !== activeTripId) {
            logger.warn({ driverId, handshakeTripId, activeTripId }, 'Ignoring handshake tripId — does not match driver’s active trip in Redis');
        }
        if (activeTripId) {
        ...
        registerDriverHandlers(socket, driverNsp, riderNsp);
    });
    return driverNsp;
}
//# sourceMappingURL=driver.namespace.js.map