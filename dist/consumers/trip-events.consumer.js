...
import { clearDriverActiveTrip, clearRiderActiveTrip, } from '../services/geo.service.js';
...
function appConfigDedupeKey(version) {
    return `gw:dedupe:appconfig:v${version}`;
}
async function shouldBroadcastAppConfig(version) {
    try {
...
// ─── Consumer bootstrap ───────────────────────────────────────────────────────
export async function startTripEventsConsumer(driverNsp, riderNsp) {
    ...
    logger.info({ topics: [config.KAFKA_TOPIC_TRIP_EVENTS, config.KAFKA_TOPIC_NOTIFICATION_OUTBOX] }, 'Kafka trip-events consumer running');
}
// ─── Handlers ─────────────────────────────────────────────────────────────────
function handleTripEvent(raw, driverNsp, riderNsp) {
    ...
    if (typeof raw === 'object' && raw !== null && raw.type === 'trip.bargain_offer') {
    ...
    const upstreamEventId = typeof event.payload.eventId === 'string'
...
function handleBargainOffer(raw, riderNsp) {
...
async function teardownTripRoom(tripId, riderId, driverId, driverNsp, riderNsp) {
    ...
    try {
        ...
        for (const s of riderSockets) {
            if (s.data.activeTripId === tripId) {
                s.data.activeTripId = undefined;
            }
        }
...
async function handleNotificationOutbox(raw, driverNsp, riderNsp) {
...
//# sourceMappingURL=trip-events.consumer.js.map