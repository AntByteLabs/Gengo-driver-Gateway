...
// ─── Consumer bootstrap ───────────────────────────────────────────────────────
export async function startOfferConsumer(driverNsp) {
    const pubsub = getRedisPubsubClient();
    await pubsub.psubscribe(OFFER_CHANNEL_PATTERN);
    pubsub.on('pmessage', (_pattern, channel, message) => {
        ...
        if (!result.success) {
            logger.warn({ channel, errors: result.error.flatten() }, 'Trip offer message validation failed');
            return;
        }
...
//# sourceMappingURL=offer.consumer.js.map